...
- 簡易 AI：優先打出能使聽牌數最多的牌
- **多人連線對戰**：為 1～4 位遠端玩家各開一個連接埠共打同一副牌，空席與離線席位自動由既有 AI 演算法接手
...
> 單人網頁模式只服務一位玩家。要讓多位玩家從不同裝置連進來打同一副牌，請改用下一節的
...
**多人連線驗收測試**（`uv run` 或 `pytest` 皆可，十節約 80 秒）：

```bash
uv run test_net_mahjong.py
```

涵蓋引擎層多席位視角、競賽模式資訊隔離、AI 代打、單人模式向下相容、
CLI 參數驗證，以及實際開兩個連接埠讓兩個 WebSocket 玩家打完一整盤。
...
`net_mahjong.py` 會**為每位遠端玩家開一個連接埠**，四個埠背後共用同一副牌；
沒有分配給真人的席位，以及離線／發呆的席位，一律沿用既有 AI 演算法（
`calculate_gates()` + `decide_play()`、見碰必碰、見吃必吃、見胡必胡、
槓依 `AI_AUTO_KONG`），不另外寫一套策略。

```bash
# 兩位玩家：席位 0 → :8001，席位 1 → :8002；席位 2、3 由 AI 補位
uv run net_mahjong.py --players 2

# 四位玩家全真人
uv run net_mahjong.py --players 4

# 指定席位與連接埠：只有 0 號與 2 號是真人
uv run net_mahjong.py --seats 0 2 --ports 9000 9001

# 離線 15 秒、發呆 60 秒後改由 AI 代打，牌局不會卡住
...
| 參數 | 預設 | 說明 |
|------|------|------|
| `--players N` | `1` | 遠端玩家人數（1–4），席位依序由 0 起算 |
...
| `--no-auto-start` | — | 玩家到齊時不自動開局，改由任一玩家按「開始對局」 |
| `--max-conns-per-seat N` | `2` | 每個席位同時允許的連線數（容許換裝置時短暫重疊） |
| `--log-level L` | `warning` | 伺服器日誌層級；內部錯誤的完整 traceback 寫在這裡，不會送給玩家 |
...
| AI 代打 | 空席、離線超時、發呆超時皆由既有 AI 演算法接手，牌局不會卡住 |
| 斷線重連 | 重新開啟網址即可回到原席位，AI 代打會自動交還 |
| 開局／下一局 | 沒有牌局在跑時（開局前、上一局結束後）任何一位玩家按下即開始，四家畫面同步重置 |
| 對局中重開 | 會作廢其他三家正在打的牌，因此需**所有在線真人各按一次**「開始對局」才生效；事件流會顯示 `(n/m 同意)` 與還在等誰，90 秒未集滿自動失效 |
| 資源上限 | 單則訊息 64 KB、每條連線 5 秒 30 則、`sync` 冷卻 2 秒、每席最多 2 條連線 |
| 事件傳輸 | 事件走 `{"t":"log"}` 依各連線游標增量推送，`state.log` 不再重複帶同樣的文字 |
| 錯誤格式 | `{"t":"error","v":{"code","message"}}`，`code` 供程式判斷（`bad_json`／`rate_limited`／`not_your_turn`／`restart_pending`／`internal_error` 等）；內部錯誤只回通用訊息，traceback 進日誌 |
...
- **Python 多人連線版**：1–4 位真人各佔一個連接埠共打一副牌；空席與離線席位由 AI 代打；
  無身分驗證，僅適合信任網路
